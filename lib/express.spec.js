...
describe('Express Plugin', () => {
    ...
    it('should create extract endpoint', () => {
        const params = { id: 1 };
        const call = adapter.extract('params.id');

        return call({ params }).then(id => {
            expect(id).toBe(params.id);
        });
    });

    it('should not crash extract endpoint with null req', () => {
        const call = adapter.extract('params.id');

        return call(null).then(id => {
            expect(id).toBe(null);
        });
    });

    it('should not crash extract endpoint empty req', () => {
        const call = adapter.extract('params.id');
